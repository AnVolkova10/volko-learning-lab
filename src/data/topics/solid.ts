...
recordAnswer('solid', true, saveLocally);`,
  ...
  exercises: [
    {
      id: "srp",
      title: "S — Find the separate responsibilities",
      prompt:
        "A function renders a topic heading and also saves it to browser storage. Which change separates its independent responsibilities?",
      code: `function showTitle(title: string) {
  ...
  localStorage.setItem('last-title', title);
      ...
      options: [
        {
          id: "rename",
          label: "Rename the function to a shorter name.",
          explanation:
            "A name does not separate rendering from storage. Both still change in the same function.",
        },
        {
          id: "separate",
          label:
            "Move saving into its own function and let the caller coordinate both.",
          explanation:
            "Yes. Rendering and persistence can now change for independent reasons. SRP is about reasons to change, not method count.",
        },
        {
          id: "one-line",
          label: "Put both statements on one line.",
          explanation:
            "Fewer lines do not mean fewer responsibilities. The two independent jobs are still mixed.",
        },
      ],
    },
    {
      id: "ocp",
      title: "O — Add behavior without rewriting the loop",
      prompt:
        "A form validator already runs a list of rule functions. You need a new rule that rejects titles longer than 80 characters. What is the best extension?",
      code: `type Rule = (value: string) => string | null;
const rules: Rule[] = [required];
// validate(value, rules) already runs every supplied rule.`,
      correctOptionId: "add-rule",
      options: [
        {
          id: "rewrite",
          label:
            "Add a title-length special case inside the shared validation loop.",
          explanation:
            "That makes the stable loop change for each new rule, even though it already accepts rule functions.",
        },
        {
          id: "add-rule",
          label: "Write a new rule function and add it to the rules array.",
          explanation:
            "Yes. The list extends behavior while the existing validation loop remains unchanged.",
        },
        {
          id: "copy",
          label: "Copy the entire validator into a second function.",
          explanation:
            "Copying duplicates stable behavior and makes future fixes harder to keep consistent.",
        },
      ],
    },
    {
      id: "lsp",
      title: "L — Keep the promised missing-result behavior",
      prompt:
        "A TitleReader promises undefined when an ID is missing. A replacement throws for missing IDs instead. Why is that a substitution problem?",
      code: `interface TitleReader {
  // Missing IDs return undefined.
  getTitle(id: string): string | undefined;
}
const title = reader.getTitle('unknown') ?? 'Not found';`,
      correctOptionId: "contract",
      options: [
        {
          id: "contract",
          label:
            "The caller expects a normal missing result, but the replacement breaks that promise.",
          explanation:
            "Yes. The fallback should keep working with every TitleReader. Type signatures alone cannot guarantee this behavioral contract.",
        },
        {
          id: "name",
          label: "The replacement needs to use the same variable name.",
          explanation:
            "Variable names do not determine substitutability. The promised inputs and outcomes do.",
        },
        {
          id: "errors",
          label: "Throwing an error is always forbidden in every interface.",
          explanation:
            "Errors are allowed when the contract allows them. This particular contract promises undefined for a missing ID.",
        },
      ],
    },
    {
      id: "isp",
      title: "I — Ask for only what you need",
      prompt:
        "A topic heading only displays a title, but its props require editing and deletion callbacks. Which SOLID principle most directly addresses this unnecessary dependency?",
      code: `type HeadingProps = {
  title: string;
  onEdit: () => void;
  onDelete: () => void;
};

function headingText(props: HeadingProps) {
  return props.title; // The callbacks are never used.
}`,
      correctOptionId: "isp",
      options: [
        {
          ...
          explanation:
            "Look at the required inputs. The function already has one clear job: returning a heading. The more direct problem is that its contract requires capabilities it never uses.",
        },
        {
          id: "ocp",
          label: "O — Open-Closed",
          explanation:
            "Open-Closed concerns extending behavior while keeping stable code unchanged. This example is about forcing callers to supply unused capabilities.",
        },
        {
          id: "lsp",
          label: "L — Liskov Substitution",
          explanation:
            "Liskov Substitution concerns an implementation breaking its base type’s promised behavior. No such substitution is shown here; focus on the oversized props contract.",
        },
        {
          id: "isp",
          label: "I — Interface Segregation",
          explanation:
            "Exactly. The heading depends on callbacks it does not need. Change HeadingProps to { title: string }. Editing controls can receive their own callbacks. SOLID principles can overlap, but ISP names this specific issue.",
        },
        {
          id: "dip",
          label: "D — Dependency Inversion",
          explanation:
            "Dependency Inversion separates a policy from implementation details through an abstraction. These callbacks are already abstractions; the heading simply should not require them.",
        },
      ],
    },
    {
      id: "dip",
      title: "D — Keep the learning rule independent of storage",
      prompt:
        "The rule is: a correct answer completes a topic. Which dependency lets that rule work with different ways of saving completion?",
      code: `type SaveCompletion = (topicId: string) => void;
// A browser implementation or a test implementation can fulfill this type.`,
      correctOptionId: "callback",
      options: [
        {
          id: "local",
          label: "Call localStorage directly inside the learning rule.",
          explanation:
            "That ties the rule to a particular storage detail. Changing storage would require changing the rule.",
        },
        {
          id: "callback",
          label:
            "Accept a SaveCompletion function and let the caller provide its implementation.",
          explanation:
            "Yes. The policy depends on the operation it needs, and the storage detail implements that contract. A plain function is enough.",
        },
        {
          id: "class",
          label:
            "Create a class for every function, but still call localStorage directly.",
          explanation:
            "Classes alone do not invert dependencies. The rule would still depend directly on the storage detail.",
        },
      ],
    },
  ],
